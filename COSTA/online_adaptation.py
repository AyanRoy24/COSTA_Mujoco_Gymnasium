...
from rlkit.envs import ENVS, ant_dir_safe
...
def do_adaptation(_config):
    ...
    for task in range(num_tasks):
        ...
        if _config["ood"]:
            ...
            env.reset_task(task)
        env_list.append(env)
        temp_vae=torch.load("run/"+_config['env_name']+"_"+str(task)+"/vae/vae.pt", weights_only=False)
    ...
    encoder=torch.load("run/"+_config['env_name']+"/context_encoder/"+_config["meta_params"][_config["meta_params"]["encoder_type"]+"_path"], weights_only=False)
    ...
    policy.load_state_dict(torch.load(_config["policy_path"], weights_only=False))
    ...
    np.save(os.path.join(logger.result_dir, "adaptation_context.npy"), np.array(context, dtype=object), allow_pickle=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--policy_path', type=str, default="run/ant-dir-safe/meta_cpq/mlp_attn/seed_0_timestamp_26-0405-015832/model/policy.pth")
