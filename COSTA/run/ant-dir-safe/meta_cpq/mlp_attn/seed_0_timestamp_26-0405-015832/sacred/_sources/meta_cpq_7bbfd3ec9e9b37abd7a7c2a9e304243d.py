    ...
    def learn(self, batch: Dict, context, task, e=0) -> Dict[str, float]:
        ...
        if not self.use_vae:
            ...
            with torch.no_grad():
                ...
                # stacked_org_obs = stacked_org_obs.to(device)
                ...
                vae_device = next(self.vae_list[org_task].parameters()).device
                sampled_actions = sampled_actions.to(vae_device)
                stacked_org_obs = stacked_org_obs.to(vae_device)
                # _, mu, logv = self.vae_list[org_task](stacked_org_obs, sampled_actions)
                ...
                index = (kld >= torch.tensor(self.kl_threshold, dtype=torch.float32).to(vae_device)).float()
                index = index.to(vae_device)
                index_sum = torch.clamp(index.sum(1), min=1.0)
                index_sum = index_sum.to(vae_device)
                qc_sampled = qc_sampled.to(vae_device)
